...
public class UserProducer {
    ...
    public void sendUserLogin(UserConfirmation userLogin) {
        log.info("Sending login: {}", userLogin);
        Message<UserConfirmation> message = MessageBuilder.withPayload(userLogin)
                .setHeader(KafkaHeaders.TOPIC, "user-login-topic")
                .setHeader(KafkaHeaders.KEY, userLogin.getUserId().toString())
                .build();
        kafkaTemplate.send(message);
    }
